...
application {
    mainModule.set("creek.connect.services.example.service")
    mainClass.set("io.github.nathensample.creek.connect.services.example.service.ServiceMain")
}
...
val buildAppImage = tasks.register<DockerBuildImage>("buildAppImage") {
    ...
    images.add("ghcr.io/nathensample/${rootProject.name}-${project.name}:latest")
    images.add("ghcr.io/nathensample/${rootProject.name}-${project.name}:${project.version}")
}
